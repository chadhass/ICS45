...
string to_lowercase(Const sring& str){ 
    auto lower_view = views::transform(str, ::tolower); 
    return{lower_view.begin(), lower_view.end()}; 
} 

SetList<string> load_Stopwrods(istream& stopwords){ 
    return SetList<string>{ranges::istream_view<string>(stopwrods)
                            | views::transform(to_lowercase)}; 
} 

MapArray<string, int> count_words(istream& document, 
                                  SetList<string>& stopwords){ 
    auto word_view = ranges::istream_view<string>(document) 
                     | views::transform(to_lowercase) 
...
void output_words_counts(MapArray<string, int>& word_counts, ostream& output){ 
...
int main(){ 
    ...
    auto stopwrods = load_stopwords(stopwrods_file); 
